...
const SellingCriteria = () => {
  ...
  const [sellingPrice, setSellingPrice] = useState(productData?.expectedSP || "$0");
  ...
  return (
    <View className="flex-1 bg-gray-100">
      ...
      <View className="flex-row justify-between items-center mb-8 px-8 py-6 mt-6">
        ...
        <Text className="text-[20px] font-[PoppinsSemiBold]">Selling Criteria</Text>
        <TouchableOpacity onPress={() => router.back()}>
      ...
      <ScrollView className="px-8">
        {productData && (
          <View className="flex-row items-center bg-white rounded-[2px] border-[1.5px] border-r-[3.5px] border-b-[3.5px] p-5 mb-6 shadow-md">
            <Image source={productData.icon} className="w-14 h-14 mr-5" resizeMode="contain" />
            <View>
              <Text className="text-[14px] font-[PoppinsSemiBold]">{productData.title}</Text>
              <Text className="text-[12px] font-[PoppinsMedium]">{productData.location}</Text>
              <Text className="text-[12px] font-[PoppinsMedium]">
                Purchased: <Text className="text-[12px] text-[#828282]">{productData.purchasedDate}</Text>
              </Text>
              <View className="flex-row items-center mt-2">
                <Text className="text-[12px] font-[PoppinsMedium]">Warranty expires in </Text>
                <View
        ...
        {/* Add Product Photos */}
        <View className="mb-6">
          <Text className="text-[16px] self-center font-[PoppinsSemiBold] mb-3 flex-row items-center justify-center">
            Add Product photos
            <Image source={require("app/assets/images/camera-icon.png")} style={{ width: 20, height: 20, marginLeft: 8 }} />
          </Text>
              ...
              .map((_, index) => (
                <TouchableOpacity
                  ...
                  className={`w-24 h-24 border-[1.5px] items-center justify-center rounded-[1px] ${
                    photoUrls[index] ? "border-black" : "border-dashed border-[#BDBDBD]"
                  }`}
                  onPress={() =>
                    photoUrls[index] ? Alert.alert("Choose Action", "Delete or Change?", [
                ...
                >
                  {photoUrls[index] ? (
                    ...
                    index === photoUrls.length ? (
                      <Text className="text-[24px] font-[PoppinsMedium] text-black">+</Text>
                    ) : null
                  )}
                </TouchableOpacity>
              ))}
        ...
        <View className="mb-6 mt-6">
          <Text className="text-[16px] font-[PoppinsMedium] mb-1">Product Condition</Text>
          <View className="border-[1.5px] rounded-[2px] shadow-sm">
            <Picker
              ...
              mode="dropdown"
              style={{ height: 60, paddingVertical: 4, fontFamily: "PoppinsSemiBold" }}
            >
        ...
        <View className="mb-6 mt-6">
          <Text className="text-[16px] font-[PoppinsMedium] mb-1">Expected Selling Price</Text>
          <View className="flex-row justify-between items-center border rounded-[2px] px-4 py-3">
            ...
            />
            <TouchableOpacity>
              <Text className="text-[12px] font-[PoppinsMedium] underline">Change</Text>
            </TouchableOpacity>
          </View>
        </View>
      </ScrollView>
      ...
      <TouchableOpacity
        ...
        onPress={() => console.log("Continue to next screen")}
      >
        <Text className="text-white text-center font-[PoppinsSemiBold] text-[14px]">Continue</Text>
      </TouchableOpacity>
    </View>
  );
};
